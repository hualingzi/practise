...
router.post('/add', function (req, res, next) {
    ...
    let db = getcon();
    db.connect();
    let sql = 'INSERT INTO wff (name,address,age) VALUES (?,?,?)';
    db.query(sql, [req.body.name, req.body.address, req.body.age], function (error, results) {
        if (error) {
            res.send({
                code: 500,
                data: '添加失败(名称已存在)'
...
router.delete('/delete', function (req, res, next) {
    ...
    let db = getcon();
    db.connect();
    let sql = 'DELETE FROM wff WHERE id = ?';
...
router.get('/check', function (req, res, next) {
    ...
    let sql = `SELECT * FROM wff`;
    let sql2;
    let count;
    if (req.query.name) {
        sql2 = `SELECT COUNT(*) FROM wff where name like '%${req.query.name}%'`;
        sql += ` where name like '%${req.query.name}%' LIMIT ${start},${pageSize}`;
    } else {
        sql2 = 'SELECT COUNT(*) FROM wff';
