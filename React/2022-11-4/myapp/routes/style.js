...
router.post('/add', function (req, res, next) {
    ...
    let db = getcon();
    db.connect();
    let sql = 'INSERT INTO style (name) VALUES (?)';
...
router.delete('/delete', function (req, res, next) {
    ...
    let db = getcon();
    db.connect();
    let sql = 'DELETE FROM style WHERE id = ?';
...
router.put('/edit', function (req, res, next) {
    let db = getcon();
    db.connect();
    let sql = 'UPDATE style SET name = ?  WHERE id = ?';
    if (!req.body.name) {
        res.send({
            code: 101,
            msg: '修改名称不能为空'
...
router.get('/check', function (req, res, next) {
    ...
    let count;
    if (req.query.name) {
        sql2 = `SELECT COUNT(*) FROM style where name like '%${req.query.name}%'`;
        sql += ` where name like '%${req.query.name}%' LIMIT ${start},${pageSize}`;
    } else {
        sql2 = 'SELECT COUNT(*) FROM style';
