...
router.post('/add', function (req, res, next) {
    ...
    let db = getcon();
    db.connect();
    let sql = 'INSERT INTO theme (name) VALUES (?)';
    db.query(sql, [req.body.name], function (error, results) {
        if (error) {
            res.send({
                code: 500,
                data: '添加失败(类型名称已存在)'
...
router.delete('/delete', function (req, res, next) {
    ...
    let db = getcon();
    db.connect();
    let sql = 'DELETE FROM theme WHERE id = ?';
...
router.put('/edit', function (req, res, next) {
    let db = getcon();
    db.connect();
    let sql = 'UPDATE theme SET name = ?  WHERE id = ?';
    if (!req.body.name) {
        res.send({
            code: 101,
            msg: '修改名称不能为空'
    ...
    db.query(sql, [req.body.name, req.body.id], function (error, results) {
        if (error) {
            res.send({
                code: 500,
                data: '系统错误' + error
...
router.get('/check', function (req, res, next) {
    ...
    let count;
    if (req.query.name) {
        sql2 = `SELECT COUNT(*) FROM theme where name like '%${req.query.name}%'`;
        sql += ` where name like '%${req.query.name}%' LIMIT ${start},${pageSize}`;
    } else {
        sql2 = 'SELECT COUNT(*) FROM theme';
